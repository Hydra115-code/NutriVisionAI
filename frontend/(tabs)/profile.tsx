...
import { estimateBodyStatsDirectly } from '../../api/geminiClient';
...
export default function ProfileScreen() {
  ...
  const handlePesoChange = (val: string) => {
    ...
    if (regex.test(val)) setEditPeso(val);
  };

  const handleAlturaChange = (val: string) => {
    const regex = /^\d{0,3}(\.\d{0,1})?$/;
    if (regex.test(val)) setEditAltura(val);
  };
  ...
  const handleGuardarEdicion = async () => {
    ...
    if (editPeso && (isNaN(pesoNum) || pesoNum < 20 || pesoNum > 300)) {
      Alert.alert("Peso inválido", "Por favor ingresa un peso real (entre 20 y 300 kg).");
      return;
    }
    if (editAltura && (isNaN(alturaNum) || alturaNum < 50 || alturaNum > 250)) {
    ...
    try {
      const response = await updateProfile(usuario, {
        ...
        peso_kg: editPeso ? pesoNum : undefined,
        altura_cm: editAltura ? alturaNum : undefined,
      ...
      if (response.ok) {
        ...
        setUsuario({
          ...
          nombre: editNombre.trim(),
          apellido: editApellido.trim(),
          peso_kg: editPeso ? pesoNum : null,
          altura_cm: editAltura ? alturaNum : null,
          objetivo: editObjetivo,
          estado_inicial: editEstado,
        });
        ...
        Alert.alert("Error", response.mensaje || "No se pudo actualizar.");
      }
    } catch {
  ...
  const procesarEscaneo = async (base64: string) => {
    ...
    try {
      ...
      if (data) {
        ...
        Alert.alert("Error", "No se pudo estimar los datos.");
      }
    } catch {
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]}>
      ...
      <Modal visible={modalEditar} transparent animationType="slide" onRequestClose={() => setModalEditar(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContainer, { backgroundColor: colors.mainCard, borderColor: effectiveBorder, borderWidth: highContrast ? 2 : 0 }]}>
            <View style={styles.modalHeader}>
              <Text style={[styles.modalTitle, { color: colors.textMain }, dynTitle]}>Editar Perfil</Text>
              <TouchableOpacity onPress={() => setModalEditar(false)}>
                <MaterialCommunityIcons name="close" size={24 * fontScale} color={colors.textSecondary} />
              </TouchableOpacity>
            </View>

            <ScrollView showsVerticalScrollIndicator={false}>
              <Text style={[styles.editLabel, { color: colors.textSecondary }, dynSmall]}>Nombre</Text>
              <TextInput
                style={[styles.editInput, { backgroundColor: isDark ? '#2d3748' : '#f1f5f9', color: colors.textMain, borderColor: effectiveBorder }, dynText]}
                value={editNombre}
              ...
              <Text style={[styles.editLabel, { color: colors.textSecondary }, dynSmall]}>Apellido</Text>
              <TextInput
                style={[styles.editInput, { backgroundColor: isDark ? '#2d3748' : '#f1f5f9', color: colors.textMain, borderColor: effectiveBorder }, dynText]}
                value={editApellido}
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.editLabel, { color: colors.textSecondary }, dynSmall]}>Peso (kg)</Text>
                  <TextInput
                    style={[styles.editInput, { backgroundColor: isDark ? '#2d3748' : '#f1f5f9', color: colors.textMain, borderColor: effectiveBorder }, dynText]}
                    value={editPeso}
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[styles.editLabel, { color: colors.textSecondary }, dynSmall]}>Altura (cm)</Text>
                  <TextInput
                    style={[styles.editInput, { backgroundColor: isDark ? '#2d3748' : '#f1f5f9', color: colors.textMain, borderColor: effectiveBorder }, dynText]}
                    value={editAltura}
              ...
              <Text style={[styles.editLabel, { color: colors.textSecondary }, dynSmall]}>Objetivo</Text>
              <TextInput
                style={[styles.editInput, { backgroundColor: isDark ? '#2d3748' : '#f1f5f9', color: colors.textMain, borderColor: effectiveBorder }, dynText]}
                value={editObjetivo}
              ...
              <Text style={[styles.editLabel, { color: colors.textSecondary }, dynSmall]}>Estado Actual</Text>
              <TextInput
                style={[styles.editInput, { backgroundColor: isDark ? '#2d3748' : '#f1f5f9', color: colors.textMain, borderColor: effectiveBorder }, dynText]}
                value={editEstado}
              ...
              >
                {guardandoEdicion
                  ...
                  : <Text style={[styles.guardarBtnText, dynText]}>Guardar cambios</Text>
                }
              </TouchableOpacity>
            </ScrollView>
          </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.headerMinimal}>
          <View style={styles.avatarCircle}>
            <Text style={[styles.avatarText, dynLarge]}>{iniciales}</Text>
          </View>
          <Text style={[styles.userName, { color: colors.textMain }, dynLarge]}>
            {usuario?.nombre || 'Usuario'} {usuario?.apellido || ''}
          </Text>
          <Text style={[styles.userSub, { color: colors.textSecondary }, dynText]}>
            {usuario?.correo || ''}
          </Text>
        </View>

        <View style={[styles.mainCard, { backgroundColor: colors.mainCard, marginBottom: 15, paddingVertical: 10, borderColor: effectiveBorder, borderWidth: highContrast ? 1 : 0 }]}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 5 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <MaterialCommunityIcons name={isDark ? "weather-night" : "weather-sunny"} size={20 * fontScale} color={colors.textMain} />
              <Text style={[{ marginLeft: 10, color: colors.textMain }, dynText]}>Modo Oscuro</Text>
            </View>
            <Switch
        ...
        <View style={[styles.mainCard, { backgroundColor: colors.mainCard, borderColor: effectiveBorder, borderWidth: highContrast ? 1 : 0 }]}>
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: colors.textSecondary }, dynSmall]}>Condición Médica</Text>
            <View style={[styles.healthBadge, { backgroundColor: usuario?.tiene_diabetes === 'si' ? '#fef2f2' : GLOBAL_COLORS.healthBadgeBg }]}>
              ...
              />
              <Text style={[styles.healthBadgeText, { color: usuario?.tiene_diabetes === 'si' ? "#ef4444" : GLOBAL_COLORS.healthBadgeText }, dynText]}>
          ...
          <View style={styles.proStatsContainer}>
            <View style={styles.proStatItem}>
              <Text style={[styles.proStatEmoji, dynLarge]}>🎯</Text>
              <Text style={[styles.proStatLabel, { color: colors.textSecondary }, dynSmall]}>Objetivo</Text>
              <Text style={[styles.proStatValue, { color: colors.textMain }, dynText]}>{usuario?.objetivo || 'No definido'}</Text>
            </View>
            <View style={styles.proStatItem}>
              <View style={[styles.statusDot, { backgroundColor: GLOBAL_COLORS.statusGreen }]} />
              <Text style={[styles.proStatLabel, { color: colors.textSecondary }, dynSmall]}>Estado</Text>
              <Text style={[styles.proStatValue, { color: GLOBAL_COLORS.statusGreen }, dynText]}>{usuario?.estado_inicial || 'Estable'}</Text>
            </View>
            <View style={styles.proStatItem}>
              <Text style={[styles.proStatEmoji, dynLarge]}>🔥</Text>
              <Text style={[styles.proStatLabel, { color: colors.textSecondary }, dynSmall]}>Racha</Text>
              <Text style={[styles.proStatValue, { color: GLOBAL_COLORS.accentOrange }, dynText]}>{usuario?.racha_inicial || 0} Días</Text>
            </View>
          </View>
          ...
          <View style={styles.statsRow}>
            <View style={styles.statBox}>
              <MaterialCommunityIcons name="weight-kilogram" size={22 * fontScale} color={colors.textSecondary} />
              <Text style={[styles.statLabel, { color: colors.textSecondary }, dynSmall]}>Peso</Text>
              <Text style={[styles.statValue, { color: colors.textMain }, dynTitle]}>{usuario?.peso_kg ? `${usuario.peso_kg} kg` : '--'}</Text>
            </View>
            <View style={styles.statBox}>
              <MaterialCommunityIcons name="ruler" size={22 * fontScale} color={colors.textSecondary} />
              <Text style={[styles.statLabel, { color: colors.textSecondary }, dynSmall]}>Estatura</Text>
              <Text style={[styles.statValue, { color: colors.textMain }, dynTitle]}>{usuario?.altura_cm ? `${usuario.altura_cm} cm` : '--'}</Text>
            </View>
          </View>
          ...
          <View style={styles.buttonContainer}>
            ...
            >
              {escaneandoCorporal
                ...
                : <MaterialCommunityIcons name="human-male-height" size={20 * fontScale} color={GLOBAL_COLORS.white} />
              }
              <Text style={[styles.primaryBtnText, dynText]}>
            ...
            <TouchableOpacity style={styles.secondaryBtn} onPress={handleAbrirEdicion}>
              <MaterialCommunityIcons name="pencil-outline" size={18 * fontScale} color={GLOBAL_COLORS.primaryGreen} />
              <Text style={[styles.secondaryBtnText, dynText]}>Editar información</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.footerActions}>
          <TouchableOpacity style={styles.footerBtn} onPress={() => Alert.alert("Términos y Condiciones", "Documento legal...")}>
            <Text style={[styles.footerBtnText, { color: colors.textSecondary }, dynSmall]}>Términos y Condiciones</Text>
          </TouchableOpacity>
          <View style={[styles.footerDivider, { backgroundColor: colors.border }]} />
          <TouchableOpacity style={styles.footerBtn} onPress={handleLogout}>
            <Text style={[styles.footerBtnText, { color: GLOBAL_COLORS.danger }, dynSmall]}>Cerrar sesión</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
